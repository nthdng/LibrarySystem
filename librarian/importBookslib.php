    ...
    if (in_array($file_extension, $allowed_extensions)) {
        ...
        foreach ($data as $row) {
            if ($count > 0) { // Skip header row
                ...
                $sqlCheckFormat = "SELECT id FROM formattbl WHERE formatName = :format";
                ...
                if ($queryCheckFormat->rowCount() == 0) {
                    ...
                    $formatId = $resultFormat['id'];
                }

                // Check if the category exists in categorytbl
                $sqlCheckCategory = "SELECT id FROM categorytbl WHERE categoryName = :category";
                ...
                $sqlCheckShelf = "SELECT id FROM shelftbl WHERE shelfLoc = :shelf";
        ...
        // Generate a summary prompt with up to 5 inserted and skipped records
        $message = "";

        // Handle inserted records
        if (!empty($insertedRecords)) {
            $totalInserted = count($insertedRecords);
            $message .= "INSERTED RECORDS:\n";
            $message .= implode(", ", array_slice($insertedRecords, 0, 5)); // Show only first 5

            if ($totalInserted > 5) {
                $message .= " and " . ($totalInserted - 5) . " more...";
            }
        ...
        if (!empty($skippedRecords)) {
            ...
            $message .= implode(", ", array_slice($skippedRecords, 0, 5)); // Show only first 5

            if ($totalSkipped > 5) {
                $message .= " and " . ($totalSkipped - 5) . " more...";
            }
        ...
        echo "<script type='text/javascript'>window.location='bookManagementlib.php';</script>";
    } else {
