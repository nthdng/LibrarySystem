...
if (isset($_POST['signup'])) {
    ...
    if ($lastInsertId) {
        ...
        try {
            ...
            // Send the email
            $mail->send();
            echo "<script>alert('Registration successful. Email with QR code sent to the recipient.'); window.location.href='facultyList.php';</script>";
        } catch (Exception $e) {
            echo '<script>alert("Registration successful, but email could not be sent. Mailer Error: ' . $mail->ErrorInfo . '");</script>';
            echo "<Script>window.location.href='facultyList.php';</Script>";
        }
    } else {
